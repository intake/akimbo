...
def _make_string_methods(utf8=True):
    ...
    if utf8 not in string_methods:
        ...
        string_methods[utf8] = set("is" + _[3:] if _.startswith("is_") else _
                                   for _ in string_methods[utf8])
    if "binary" not in string_methods:
        ...
        string_methods['standard'] = {
            'startswith', 'replace', 'string_is_ascii', 'replace_substring',
            'replace_substring_regex', 'split_pattern', 'extract_regex',
            'count_substring', 'count_substring_regex', 'endswith',
...
def _decode(layout, continuation, encoding='utf-8', **kwargs):
    if layout.is_ListType and layout.parameter("__array__") == "bytestring":
        ...
        out = ak.with_parameter(
            layout, "__array__", "string", highlevel=False
        )
        out._content = content   # the "out" Python object is a copy
        return out
    return layout
